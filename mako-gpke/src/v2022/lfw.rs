...
/// GPKE 2022 (MaKo 2022, gueltig ab 01.10.2023): Lieferantenwechsel
///
/// Same 6-state machine as v2025, but with pre-LFW24 fristen:
/// - Pruefungsfrist NB: 5 WT (not 1 WT as in v2025)
/// - Widerspruchsfrist LFA: 5 WT (not 24h as in v2025)
/// - No 24h switch capability
...
pub enum LfwState {
	...
	/// Widerspruchsfrist: 5 WT (longer than v2025's 24h)
...
pub fn reduce(state: LfwState, event: LfwEvent) -> Result<ReducerOutput<LfwState>, ProzessFehler> {
	match (state, event) {
		...
		(
			...
			// v2022 Widerspruchsfrist: 5 WT (simplified to lieferbeginn here;
			// actual calculation via mako-fristen in production)
