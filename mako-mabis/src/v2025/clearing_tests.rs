...
fn clearingliste() -> UtilmdClearingliste {
	UtilmdClearingliste {
		eintraege: vec![ClearingEintrag {
			...
			nb_wert: "3500 kWh".to_string(),
			lf_wert: Some("3200 kWh".to_string()),
		}],
	}
}
...
fn antwortliste() -> UtilmdClearingliste {
	UtilmdClearingliste {
		eintraege: vec![ClearingEintrag {
			...
			feld: "Jahresverbrauch".to_string(),
			nb_wert: "3500 kWh".to_string(),
			lf_wert: None, // accepted NB value
		}],
	}
}
...
fn idle_plus_liste_transitions_to_gesendet() {
	let out = reduce(
		ClearingState::Idle,
		ClearingEvent::ListeEmpfangen(clearingliste()),
	)
...
#[test]
fn gesendet_plus_antwort_transitions_to_antwort_empfangen() {
	let state = ClearingState::ListeGesendet {
		eintraege: clearingliste().eintraege,
	};
	let out = reduce(state, ClearingEvent::AntwortEmpfangen(antwortliste()))
		.expect("should succeed");
	assert!(matches!(
		out.state,
		ClearingState::AntwortEmpfangen { .. }
	));
	assert!(out.nachrichten.is_empty());
}
...
fn idle_cannot_receive_antwort() {
	let result = reduce(
		ClearingState::Idle,
		ClearingEvent::AntwortEmpfangen(antwortliste()),
	);
	assert!(matches!(
		result,
